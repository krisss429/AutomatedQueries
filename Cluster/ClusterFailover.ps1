# Use this powershell script to perform a local cluster failover
# Format :  .\ClusterFailover [node to failover from] [node to failover to] [node to failover to must be inactive? Yes/No (default = Yes)]
# Example:  .\ClusterFailover camakrsqln1 camakrsqln2 No
#
#
$logdate = (get-date).tostring('yyyy-MM-dd_HH-mm-ss')
$log = "C:\ClusterFailover_" + $logdate + ".log"            
...
if ($args.length -ne 2 -and $args.length -ne 3)
{
	$now = (get-date).tostring('HH:mm:ss -')            
    	...
    	out-file -filepath $log -inputobject "$now Format :  .\ClusterFailover [node to failover from] [node to failover to] [node to failover to must be inactive? Yes/No (default = Yes)]" -Append
    	out-file -filepath $log -inputobject "$now Example:  .\ClusterFailover camakrsqln1 camakrsqln2 No" -Append
    	out-file -filepath $log -inputobject "$now Terminating script." -Append
...
if ($args.length -eq 3)
{
	$InactiveNodeVerification = $args[2]
}
else
{
...
{
	out-file -filepath $log -inputobject "$now Input parameters are: $FromNode and $ToNode" -Append
}
else
{
	out-file -filepath $log -inputobject "$now Input parameters are: $FromNode and $ToNode and $InactiveNodeVerification" -Append
}
...
#
$now = (get-date).tostring('HH:mm:ss -')
out-file -filepath $log -inputobject "$now Validating first parameter $FromNode (node to failover from)" -Append

if ($NodeArray -notcontains $FromNode)
{
	out-file -filepath $log -inputobject "$now $FromNode is not a valid node name." -Append
	out-file -filepath $log -inputobject "$now First parameter should be one of the following: $ActiveNodeArray" -Append
    	out-file -filepath $log -inputobject "$now Terminating script." -Append
...
#
$now = (get-date).tostring('HH:mm:ss -')
out-file -filepath $log -inputobject "$now Validating second parameter $ToNode (node to failover to)" -Append

if ($NodeArray -notcontains $ToNode)
{
	out-file -filepath $log -inputobject "$now $ToNode is not a valid node name." -Append
	out-file -filepath $log -inputobject "$now Second parameter should be one of the following: $InactiveNodeArray" -Append
    	out-file -filepath $log -inputobject "$now Terminating script." -Append
...
{
	$now = (get-date).tostring('HH:mm:ss -')
	out-file -filepath $log -inputobject "$now Validating third parameter $InactiveNodeVerification (node to failover to must be inactive?)" -Append
	...
	if ($InactiveNodeVerification -ne "Yes" -and $InactiveNodeVerification -ne "No")
	{
		...
		out-file -filepath $log -inputobject "$now Third parameter should be one of the following: Yes No" -Append
	    	out-file -filepath $log -inputobject "$now Terminating script." -Append
...
# Open new log file
#
$log = "C:\ClusterFailover_" + $ClusterName + "_" + $logdate + ".log"
