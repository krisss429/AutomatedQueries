...
# Format :  .\RemoteClusterReset [cluster name] [node 1] [node 2]
# Example:  .\RemoteClusterReset camakrsqlcls1 camakrsqln1 camakrsqln2
#
#
$logdate = (get-date).tostring('yyyy-MM-dd_HH-mm-ss')
$log = "C:\RemoteClusterReset_" + $logdate + ".log"            
...
if ($args.length -ne 3)
{
	$now = (get-date).tostring('HH:mm:ss -')            
    	...
    	out-file -filepath $log -inputobject "$now Format :  .\RemoteClusterReset [cluster name] [node 1] [node 2]" -Append
    	out-file -filepath $log -inputobject "$now Example:  .\RemoteClusterReset camakrsqlcls1 camakrsqln1 camakrsqln2" -Append
    	out-file -filepath $log -inputobject "$now Terminating script." -Append
...
# Open new log file
#
$log = "C:\RemoteClusterReset_" + $ClusterName + "_" + $logdate + ".log"
